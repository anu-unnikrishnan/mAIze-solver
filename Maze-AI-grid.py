...
def update_maze(grid, playerpos):
    for i in range(n):
        for j in range(n):
...
def show(maze, playerpos, visitedcellsrow, visitedcellscol):
    plt.grid('on')
    nrows = n
    ncols = n
    ax = plt.gca()
...
def move_player(grid, playerpos, totalreward, qtable, visitedcellsrow, visitedcellscol):
    flag = 0
    while flag == 0:
        reward = 0
        ...
        #if all values are zero in that row, move randomly
        if qflag == 0:
...
while ismazeok == 'n':
    ...
    visitedcellsrow = [0]
    visitedcellscol = [0]
    show(grid, playerpos, visitedcellsrow, visitedcellscol)
    ismazeok = input("\nIs this maze ok? (y/n) \n")
...
while episodes <= max_episodes: 
    ...
    while keepmoving == 'y':
        totalreward = move_player(grid, playerpos, totalreward, qtable, visitedcellsrow, visitedcellscol) #move player 
        ...
        if playerpos[0] == n-1 and playerpos[1] == n-1: #if reached the end, we're done!
            keepmoving = 'n'
        if totalreward <= -100*(n**2): #if moving around for too long - this depends on size of maze 
            unsolved = 1
            break
